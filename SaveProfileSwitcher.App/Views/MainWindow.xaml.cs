...
public partial class MainWindow : Window
{
    ...
    private void AddProfile_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (dialog.ShowDialog() == true && dialog.CreatedProfile != null && DataContext is MainViewModel vm)
        {
    ...
    {
        if (sender is Button button && button.DataContext is GameConfig game && DataContext is MainViewModel vm)
        {
            if (new EditGameDialog(game) { Owner = this }.ShowDialog() == true)
            {
                vm.SaveData();
    ...
    {
        if (sender is Button button && button.DataContext is GameConfig game && button.Tag is ComboBox selector && selector.SelectedItem is UserProfile profile)
        {
            try
            {
    ...
    {
        if (sender is Button button && button.DataContext is GameConfig game && !string.IsNullOrWhiteSpace(game.EmulatorConfig?.ExecutablePath))
        {
            try
            {
                Process.Start(new ProcessStartInfo(game.EmulatorConfig.ExecutablePath) { UseShellExecute = true });
            }
            catch (Exception ex)
            {
